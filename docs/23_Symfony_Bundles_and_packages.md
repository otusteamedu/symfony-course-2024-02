...
11. Исправляем класс `App\Controller\Api\CreateUser\v5\CreateUserManager`
    ```php
    <?php
    ...
    ```
12. Выполняем запрос Add user v5 из Postman-коллекции v10 и проверяем, что данные поступают в Graphite
...
2. Создаём класс `FeedBundle\Controller\ServerApi\GetFeed\v1\Controller`
    ...
    ```
4. В классе `App\Controller\Api\GetFeed\v1\Controller` заменяем `FeedFacade` на `FeedClient`
