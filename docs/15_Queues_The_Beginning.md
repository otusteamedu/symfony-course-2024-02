...
3. Добавляем класс `App\Controller\Api\AddFollowers\v1\Controller`
    ...
    ```
4. Заходим в контейнер командой `docker exec -it php-1 sh` и сбрасываем кэш метаданных Doctrine командой
...
2. В файл `docker-compose.yml` добавляем новый сервис:
    ...
    ```
3. Перезапускаем контейнеры с пересборкой
    ```shell
    docker-compose stop
    docker-compose up -d --build
    ```
4. Заходим в контейнер командой `docker exec -it php-1 sh` и устанавливаем пакет `php-amqplib/rabbitmq-bundle`
...
7. В файл `config/services.yaml` добавляем новый сервис
    ...
    ```
8. Заходим в контейнер командой `docker exec -it php-1 sh`. Дальнейшие команды выполняются из контейнера
